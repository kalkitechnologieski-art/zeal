...
export const POST = withErrorHandler(async (req: Request) => {
  ...
  const systemPrompt = `You are ${session.aiConsultant.name}, a ${session.aiConsultant.persona || "helpful"} AI ${session.aiConsultant.category}. Respond to the user's question in a clear, empathetic, and professional manner.`;
