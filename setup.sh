#!/usr/bin/env bash
# ==============================================================================
# PROJECT ZEAL — HIGH-END EXPLORE & DISCOVERY HUB (REAL-TIME SUPABASE FETCH)
# ==============================================================================
set -euo pipefail
...
echo -e "${INFO} 1. Building High-End Real-Time Explore Page..."
mkdir -p apps/web/app/explore

cat << 'EOF' > apps/web/app/explore/page.tsx
"use client";
...
echo -e "${INFO} 2. Purging Turbopack Cache & Running Builds..."
rm -rf apps/web/.next
rm -rf apps/web/node_modules/.cache/turbopack || true
npx tsc --noEmit --project apps/web/tsconfig.json || { echo -e "${ERROR} Type check failed!"; exit 1; }
npm --workspace web run build || { echo -e "${ERROR} Build failed!"; exit 1; }

echo -e "${SUCCESS} ====================================================================="
echo -e "${SUCCESS} EXPLORE PAGE FULLY UPGRADED!"
echo -e "${SUCCESS} - Real-time fetching from profiles & ai_profiles"
echo -e "${SUCCESS} - Interactive searching & category filtering"
echo -e "${SUCCESS} - Type filtering toggle (All, Human Masters, AI Avatars)"
echo -e "${SUCCESS} - Strictly formatted in INR (₹)"
echo -e "${SUCCESS} Run 'npm --workspace web run dev' to visit /explore!"
echo -e "${SUCCESS} ====================================================================="